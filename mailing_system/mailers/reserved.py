"""Mailer client for sending reserved attendance confirmation emails."""
...
logger = get_logger("mailers.reserved")


class ReservedMailer(ZeptoMailerBase):
    """Client service for sending reserved spot confirmation emails via ZeptoMail."""
    ...
    def send_reserved(self, applicant: Applicant) -> DeliveryResult:
        """Compose payload and dispatch a reserved spot confirmation email."""
        ...
        subject = f"Your Reserved Spot for the {self.event_config.name}"

        payload = self._build_common_payload(
            to_email=applicant.email,
            to_name=applicant.name,
            subject=subject,
            html_body=ReservedConfirmationTemplate.render_html(applicant, self.event_config),
            text_body=ReservedConfirmationTemplate.render_plain(applicant, self.event_config),
            inline_images=[
        ...
        if result.status == "SUCCESS":
            logger.info(
                "✅ SUCCESS: Sent reserved confirmation to '%s' [Solo: %s | Team: %s] (ReqID: %s)",
                applicant.email,
                applicant.is_solo,
                applicant.team_name,
                result.request_id,
    ...
    def send_batch(
        ...
        failed_report_path: Path = LOG_DIR / "failed_reservations.json",
        success_report_path: Path = LOG_DIR / "success_reservations.json",
    ) -> Tuple[List[DeliveryResult], List[DeliveryResult]]:
        """Send reserved confirmations to a list of applicants in batch with reports."""
