...
from mailing_system.db.repository import ensure_sent_at_column, resolve_db_path
...
class SheetsSyncer:
    ...
    def sync_to_db(self) -> bool:
        ...
        keep_cols = [col for col in (name_col, email_col, team_col) if col is not None]
    ...
    def _ensure_schema(self, db_path: Path) -> bool:
        try:
            added = ensure_sent_at_column(db_path, self.db.table_name)
            if added:
                logger.info("Added 'sent_at' column to table '%s'.", self.db.table_name)
        except Exception:
            logger.exception("Failed to ensure 'sent_at' column on '%s'.", self.db.table_name)
        return True
